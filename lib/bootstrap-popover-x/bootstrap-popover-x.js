...
(function (factory) {
    ...
    PopoverX.prototype = $.extend({}, $.fn.modal.Constructor.prototype, {
        ...
        hide: function () {
            ...
            $(document).off('click.popoverx');
            $(document).off('keyup.popoverx');
        },
        show: function (skipValidation) {
            ...
            $(document).on('click.popoverx', function (e) {
                var $tar = $(e.target);
                if ($tar.closest(self.$target).length === 0 &&
                    $tar.closest($dialog).length === 0) {
                    $dialog.popoverX('hide');
                }
            });
            $(document).on('keyup.popoverx', function (e) {
                if (e.keyCode === 27) {
                    $dialog.popoverX('hide');
                }
            });
