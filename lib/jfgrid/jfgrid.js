...
*
*coldata:label,name,width,align,sort,frozen,formatter
*/
...
(function ($) {
    ...
    $.extend($.jfGrid, {
        ...
        reload: function ($self) {
            ...
            if (dfop.isPage) {
                ...
                dfop.param.rows = dfop.pageparam.rows;
                dfop.param.page = dfop.pageparam.page;
                dfop.param.sidx = dfop.pageparam.sidx;
                dfop.param.sord = dfop.pageparam.sord;
                $.jfGrid.ajaxLoad($self, dfop.url, dfop.param, function (res) {
                    if (jx.toBoolean(res.success)) {
                        dfop.rowdatas = res.rows;
                        dfop.pageparam.page = res.page;
                        dfop.pageparam.records = res.records;
                        dfop.pageparam.total = res.total;
                        $.jfGrid.renderData($self);
                    }
                    else {
                        dfop.rowdatas = [];
                        ...
                        dfop.pageparam.records = 0;
                        dfop.pageparam.total = 0;
                        $self.find('.jfgrid-scrollarea').html('<pre>' + res.msg + '</pre>');
                        $self.find('.jfgrid-nodata-img').hide();
                    }
                    if (!!dfop.onRenderComplete) {
                        dfop.onRenderComplete(res.rows);
                    }
                });
            } else {
                $.jfGrid.ajaxLoad($self, dfop.url, dfop.param, function (res) {
        ...
        renderRowData: function ($self, data) {
            ...
            for (var i = 0; i < len; i++) {//行循环
                ...
                if (dfop.isShowNum) {
                    ...
                    $letf.append($dcell);
                    $lastCell = $dcell;
                    if (isLastRow && !dfop.isLastBorder) {
                        $dcell.css('border-bottom-width', 0);
                    }
                }
                if (dfop.isMultiselect) {
                    // 如果是多选绑定字段
                ...
                for (var m = 0, ml = dfop._colFrozenModel.length; m < ml; m++) {// 左侧固定列
                    var $dcell = $('<div class="jfgrid-data-cell" rownum="' + rownum + '"  colname="' + dfop._colFrozenModel[m].name + '" style="width:' + dfop._colFrozenModel[m].width + 'px;text-align:' + (dfop._colFrozenModel[m].align || 'left') + ';"  title="' + (_row[dfop._colFrozenModel[m].name] || "") + '" ></div>');
                    if (m == ml - 1) {
                        $lastCell = $dcell;
                    }
                    if (isLastRow && !dfop.isLastBorder) {
                ...
                for (var n = 0, nl = dfop._colModel.length; n < nl; n++) {// 右侧滚动显示区域
                    var $dcell = $('<div class="jfgrid-data-cell" rownum="' + rownum + '" colname="' + dfop._colModel[n].name + '" style="width:' + dfop._colModel[n].width + 'px;text-align:' + (dfop._colModel[n].align || 'left') + ';" title="' + (_row[dfop._colModel[n].name] || "") + '"  ></div>');
                    if (n == nl - 1) {
            ...
            if (len == 0) {// 没有数据显示背景图片
                ...
                if (dfop.isAutoHeight) {
                    ...
                    var _bottom = $self.css('padding-bottom');
                    if (dfop.footerrow && !dfop.isPage) {
                        _height = parseInt(_height) + 29;
                    }
                    $self.css({'height': (parseInt(_height) + parseInt(_top) + parseInt(_bottom))});
                }
            }
            ...
            if (!!node.editType) {
                switch (node.editType) {
                    ...
                    case 'date':
                        ...
                        $dcell.html($input);
                        $input.jxdate(node.editOp||{});
                        break;
                    case 'select':
                        var $input = $('<select class="form-control"></select>');//.val(value || '');
                        $input.css({'text-align': node.align});
                    ...
                    case 'checkbox':
                        ...
                        break;
                    case 'switch':
                        var $input = $('<div style="position: absolute;top: 0;left: 0;width: 100%;height: 100%;background: #fff;" >' +
        ...
        ajaxLoad: function ($self, url, param, callback) {
            ...
            $.ajax({
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    callback({code: 500});
                    var html = jx.getBody(XMLHttpRequest.responseText);
                    $self.find('.jfgrid-scrollarea').html(html);
                    $self.find('.jfgrid-nodata-img').hide();
                },
                beforeSend: function () {
    ...
    $.fn.jfGrid = function (op) {
        ...
        var dfop = {
            ...
            isPage: true,                 // 是否分页默认是不分页（目前只支持服务端分页）
            pageparam: {
                rows: 10,                 // 每页行数
                page: 1,                  // 当前页
                sidx: '',                 // 排序列
