...
(function ($) {
    ...
    jx.plugin({
        ...
        instance: function ($ele, ops) {
            return Chosen($ele, ops);
        },
        options: function ($ele) {
            return jx.parseOptions($ele, ['url', 'method', 'width', {params: 'object'}]);
        },
        defaults: {
            url: null,
            method: 'get',
        ...
        onBeforeInit:function (e) {//注册依赖
            if (document.querySelector('select.jxchosen')) {
                jx.depend('chosen');
                //调整验证选项回调
        ...
        onInit:function (e) {//实例化
            $(e.target).find('select.jxchosen').jxchosen();
        }
    });
